...
export default function Menu() {
    ...
    const createFoodCategoryData = (): readonly FoodCategoryModel[] => [
        {key: 0, label: 'Frühstück', items: [{key: 0, label: "Rühreier"}, {key: 1, label: "Brötchen"}]},
        {key: 1, label: 'Mittagessen', items: [{key: 2, label: "Döner"}]},
        {key: 2, label: 'Abendessen', items: [{key: 3, label: "Tomaten"}]},
        {
            key: 3,
            label: 'Getränke',
            items: [{key: 4, label: "Wasser"}, {key: 4, label: "Kaffee"}, {key: 4, label: "Tee"}, {
                key: 4,
                label: "Cola"
            }]
        }
    ...
    const [foodData] = useState<readonly FoodCategoryModel[]>(createFoodCategoryData());
    return (
        <Stack className="mb-4">
            ...
            {foodData.map((data) => {
                return (
                    <FoodCategory key={data.key} label={data.label} items={data.items}/>
                );
            })}
